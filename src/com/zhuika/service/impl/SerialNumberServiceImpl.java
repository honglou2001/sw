...
public class SerialNumberServiceImpl implements SerialNumberService {
    ...
    @Resource
    private ISerialnumberFeePacketDao serialnumberFeePacketDao;
...
//		byte b[] = HexString2Bytes(info);
	...
	public void updateSerialNumberAction(Serialnumber serialNumber,HashMap<String, String> mapSerial) {
		...
		else if(action.equals("7111"))  //发送主服务器ip地址及端口
		{
			cmd = "7111";
		}
		else if(action.equals("customfull"))  //辅助指令，发送自定义内容短信
		{
			cmd = "customfull";
		}	
		String info="";
...
//			}  //utf-8  unicode  //FEFF   
		...
		else if(cmd.equals("customfull")){
			
			String customcmd  = "";
			String cusdataHexStr = "";
			boolean isHex = true;
			if(mapSerial.containsKey("customcmd"))
			{
				customcmd = mapSerial.get("customcmd");
			}
			if(mapSerial.containsKey("cusdataHexStr"))
			{
				cusdataHexStr = mapSerial.get("cusdataHexStr");
			}
			
			if(mapSerial.containsKey("cusIsHex"))
			{
				String cusval = mapSerial.get("cusIsHex");
				if(cusval.equals("0"))
				{
					isHex = false;
				}
			...
			info = ByteConverter.GetSendData(serialNumber.getSerialnumber(),customcmd,cusdataHexStr,isHex);			
			ejbproxy.saveToMq(serialNumber.getSerialnumber(), customcmd, info, usrid, "自定义指令",param);
		}
		else if(cmd.equals("7111")){
			String serverip = "";
			String serverport = "";
			if(mapSerial.containsKey("serverip"))
			{				
				serverip = mapSerial.get("serverip");
			
			}
			if(mapSerial.containsKey("serverport"))
			{				
				serverport = mapSerial.get("serverport");
			
			}
			String ipAndPort = serverip+":"+serverport;
			
			info = ByteConverter.GetSendData(serialNumber.getSerialnumber(),cmd,ipAndPort,false);			
			ejbproxy.saveToMq(serialNumber.getSerialnumber(), cmd, info, usrid, "设置主服务器IP及端口",ipAndPort);			
		}   //通讯录：
...
//		}
	...
	// 从十六进制字符串到字节数组转换
		...
		@Override
		public void AddSerialnumberFeepacket(
				SerialnumberFeepacket serialnumberFeepacketInfo) {
			// TODO Auto-generated method stub
			this.serialnumberFeePacketDao.AddSerialnumberFeepacket(serialnumberFeepacketInfo);
		}



		@Override
		public void UpdateSerialnumberFeepacket(
				SerialnumberFeepacket serialnumberFeepacketInfo) {

			this.serialnumberFeePacketDao.UpdateSerialnumberFeepacket(serialnumberFeepacketInfo);
		...
		public void DeleteSerialnumberFeepacket(String id) {
			// TODO Auto-generated method stub
			this.serialnumberFeePacketDao.DeleteSerialnumberFeepacket(id);
		}

		@Override
		public SerialnumberFeepacket findSerialnumberFeepacket(String id) {
			// TODO Auto-generated method stub
			return this.serialnumberFeePacketDao.findSerialnumberFeepacket(id);
		}

		@Override
		public int GetSerialnumberFeepacketCount(HashMap<String, String> map) {
			// TODO Auto-generated method stub
			return this.serialnumberFeePacketDao.GetSerialnumberFeepacketCount(map);
		}

		@Override
		public List<SerialnumberFeepacket> ListSerialnumberFeepacket(
				int offset, int length, HashMap<String, String> map) {
			// TODO Auto-generated method stub
			return  this.serialnumberFeePacketDao.ListSerialnumberFeepacket(offset,length,map);
		}
		...
		public List<Serialnumber> ListSerialnumberAll(int offset, int length,
				HashMap<String, String> map) {
			// TODO Auto-generated method stub
			return this.serialNumberDao.ListSerialnumberAll(offset, length, map);
		}
		...
		public SerialnumberFee findSerialnumberFee(String id) {
			// TODO Auto-generated method stub
			return this.serialnumberFeeDao.findSerialnumberFee(id);
		}
		...
		public void UpdateSerialnumberFee(SerialnumberFee serialnumberFeeInfo) {
			this.serialnumberFeeDao.UpdateSerialnumberFee(serialnumberFeeInfo);
