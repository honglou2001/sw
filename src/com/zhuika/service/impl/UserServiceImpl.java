...
//		usrrelate.setFsnid(serialNumber.getId());
	...
	public int RemoveUsrRelation(String serialnumid,String userid)
	{
		UserSnrelateDaoIml  usrrelatedao = new UserSnrelateDaoIml();
		
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("FUserIDStr", userid);
		map.put("FSNIDStr", serialnumid);
		
		List<UserSnrelate>  listRelate = usrrelatedao.GetAll(0, 10, map);
		
		if(listRelate!=null && listRelate.size()>0)
		{
			String id = listRelate.get(0).getFrelateid();
			usrrelatedao.Delete(id);			
			return 1;
		}
		else
		{
...
//	
	...
	public void saveToContact(UserWatch postUser,String contactName,String contactNum,int fproperties,int dialtype,Serialnumber serialNumTo)
	{
		...
		if(serialNumTo!=null && serialNumTo.getFdatastatus()!=null)
		{
			//表示为全局为直拨，不考虑是否同网还是异网
			if((serialNumTo.getFdatastatus() & 2) ==2)
			{
				dialtype = 0 ;
			}
		}
